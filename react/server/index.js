...
app.listen(port, function() {
  ...
  app.get('/login', (req, res) => {
    db.query(`select * from data inner join users on users.email = data.email where users.email = '${req.query.email}'`)
      .then(({rows}) => {
        res.send(rows);
      })
      .catch((err) => {
  ...
  app.post('/data', (req, res) => {
    ...
    db.query(`update data set editableText = '${req.body.editData}' where id = ${id}`)
      .then(({rows}) => {
        res.send(rows);
      })
      .catch((err) => {
    ...
    db.query(`INSERT INTO Users (email) VALUES ('${req.body.email}')`)
      .then(({rows}) => {
        ...
        .then(({rows}) => {
          ...
          Aenean mollis, leo et consectetur tincidunt, ', '${req.body.email}', 'SECOND_EDITABLE_FIELD');`)
            .then(({rows}) => {
              db.query(`INSERT INTO data (data1, email, editableText) VALUES ('neque porttitor sapien, luctus auctor magna mi vel ante.  ', '${req.body.email}', 'THIRD_EDITABLE_FIELD');`)
              .then(({rows}) => {
                ...
                Praesent sollicitudin, quam nec tempor molestie, elit  ', '${req.body.email}', 'FORTH_EDITABLE_FIELD');`)
                .then(({rows}) => {
                  ...
                  eu elementum elit varius. Aenean libero dolor, vulputate quis  ', '${req.body.email}', 'FIFTH_EDITABLE_FIELD');`)
                    .then(({rows}) => {
                      db.query(`INSERT INTO data (data1, email, editableText) VALUES (' vitae, laoreet sit amet felis. Pellentesque sit amet laoreet sem, sit amet scelerisque arcu. Praesent pulvinar mauris a finibus porttitor. Aenean sodales convallis mi, quis tincidunt ante consequat.', '${req.body.email}', 'SIXTH_EDITABLE_FIELD');`)
                      .then(({rows}) => {
                        res.send(rows);
                      })
                      .catch((err) => {
                  ...
                  .catch((err) => {
                    console.error("I am in /newData", err);
                  })
              })
              .catch((err) => {
                console.error(err);
